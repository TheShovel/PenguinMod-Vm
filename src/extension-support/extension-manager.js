...
const defaultBuiltinExtensions = {
  ...
  theshovelcanvaseffects: () =>
    require("../extensions/theshovel_canvasEffects"),
  // shovellzcompresss: ...
  ...
  // theshovelconsole: ...
  theshovelconsole: () => require("../extensions/theshovel_console"),
...
class ExtensionManager {
  ...
  async loadExtensionURL(extensionURL, oldHash = "") {
    ...
    if (extensionURL.includes("penguinmod.site")) {
      alert(
        "Extensions using penguinmod.site are deprecated, please swap them over to use penguinmod.com instead.",
      );
    }
    ...
    if (
      oldHash &&
      oldHash !== newHash &&
      this.securityManager.shouldUseLocal(extensionURL)
    )
      return Promise.reject("useLocal");

    if (sandboxMode === "unsandboxed") {
      const { load } = require("./tw-unsandboxed-extension-runner");
      const extensionObjects = await load(blobUrl, this.vm).catch((error) =>
  ...
  refreshDynamicCategorys() {
    if (!this._loadedExtensions)
      return Promise.reject("_loadedExtensions is not readable yet");
  ...
  _normalize(thing, to) {
    switch (to) {
      case "string":
        return Cast.toString(thing);
      case "bigint":
      case "number":
        return Cast.toNumber(thing);
      case "boolean":
        return Cast.toBoolean(thing);
      case "function":
    ...
    switch (blockInfo.blockType) {
      ...
      case BlockType.BUTTON:
        if (!blockInfo.opcode && !blockInfo.func) {
          throw new Error(
            `Missing opcode or func for button: ${blockInfo.text}`,
          );
        }
      ...
      default: {
        ...
        blockInfo.func = (args, util, visualReport) => {
          ...
          for (const arg in realBlockInfo.arguments) {
            ...
            if (realBlockInfo.arguments[arg].exemptFromNormalization === true)
              continue;
            ...
            if (
              typeof menus[realBlockInfo.arguments[arg].menu]?.variableType ===
              "string"
            )
              continue;
            if (!(typeof args[arg] === expected))
              args[arg] = this._normalize(args[arg], expected);
          }
          // TODO: filter args using the keys of realBlockInfo.arguments? maybe only if sandboxed?
          const returnValue = callBlockFunc(args, util, realBlockInfo);
          if (!visualReport && (returnValue?.value ?? false))
